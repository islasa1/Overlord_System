...
//*****************************************************************************
  ...
  if(ui8Status == I2CM_STATUS_SUCCESS) {
    MPU9X50DataAccelGetFloat((tMPU9X50*) pvCallbackData, &g_pfAccel[0], &g_pfAccel[1], &g_pfAccel[2]);
    MPU9X50DataGyroGetFloat((tMPU9X50*) pvCallbackData, &g_pfGyro[0], &g_pfGyro[1], &g_pfGyro[2]);
    ...
    CalculatePitchRoll(g_pfAccel[MPU9X50_X_AXIS], g_pfAccel[MPU9X50_Y_AXIS], g_pfAccel[MPU9X50_Z_AXIS], &g_pfHead[MPU9X50_ROLL], &g_pfHead[MPU9X50_PITCH]);

    g_pfHead[MPU9X50_YAW] = MagnetoHeadingCompute(g_pfMag[MPU9X50_X_AXIS], g_pfMag[MPU9X50_Y_AXIS], g_pfMag[MPU9X50_Z_AXIS], g_pfHead[MPU9X50_ROLL]*DEG_TO_RAD, g_pfHead[MPU9X50_PITCH]*DEG_TO_RAD) * RAD_TO_DEG;
  }
...
AK8963DataAppCallback(void *pvCallbackData, uint_fast8_t ui8Status) {
    ...
    if(ui8Status == I2CM_STATUS_SUCCESS) {
        AK8963DataMagnetoGetFloat((tAK8963*) pvCallbackData, &g_pfMag[0], &g_pfMag[1], &g_pfMag[2]);
